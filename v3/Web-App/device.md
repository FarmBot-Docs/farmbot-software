...
content="Press the `+` in `[+] ADVANCED` to expand the advanced settings section."
%}
...
include callout.html
type="warning"
title="Encoder scaling is still experimental"
content="Please note that encoder scaling is still experimental and using the calculated factor of 56 may produce undesired results. In the meantime, **using the default value of 100 for each axis** will work more reliably."
%}
...
The default Encoder Missed Step Decay value for each axis is 10. Changing this value is only necessary if you are using much higher resolution encoders and/or a slower processor and/or driving FarmBot at very high speeds (greater than 2,000 steps/s), all of which would cause more encoder pulses to go undetected.
